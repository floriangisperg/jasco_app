...
def preprocess_time_series_data(df):
    """Clean and prepare time series data with time conversion"""
    ...
    # Convert column names to time in hours if they're numeric
    hours_columns = {}
    for col in numeric_df.columns:
        try:
            # Try to extract a numeric value from the column name
            time_val = float(col)
            # If the time is in seconds (large values), convert to hours
            if time_val > 1000:  # Likely in seconds
                hours_val = time_val / 3600  # Convert seconds to hours
                hours_columns[col] = f"{hours_val:.3f}"
            # If the time is in minutes (medium values), convert to hours
            elif time_val > 100:  # Likely in minutes
                hours_val = time_val / 60  # Convert minutes to hours
                hours_columns[col] = f"{hours_val:.3f}"
            else:
                # Already in hours or a small value
                hours_columns[col] = col
        except:
            # Not a numeric column name, keep as is
            pass

    # Rename columns if conversions were made
    if hours_columns:
        numeric_df = numeric_df.rename(columns=hours_columns)
...
def blank_subtraction_ui(df):
    ...
    # Use actual column names for selection, but try to convert to hours for display
    ...
    for col in column_options:
        try:
            ...
            time_val = float(col)
            # If the time is in seconds (large values), display as hours
            if time_val > 1000:  # Likely in seconds
                hours_val = time_val / 3600  # Convert seconds to hours
                display_options.append(f"{col} ({hours_val:.3f}h)")
            # If the time is in minutes (medium values), display as hours
            elif time_val > 100:  # Likely in minutes
                hours_val = time_val / 60  # Convert minutes to hours
                display_options.append(f"{col} ({hours_val:.3f}h)")
            else:
                # Already in hours or a small value
                display_options.append(f"{col}h")
        except:
            # Not a numeric column name
            display_options.append(str(col))

    # Create mapping of display options back to actual column names
    options_map = dict(zip(display_options, column_options))
    ...
    if blank_method == "Single Timepoint":
        display_timepoint = st.sidebar.selectbox(
        ...
        blank_timepoint = options_map[display_timepoint]
        ...
        if st.sidebar.checkbox("Preview Blank Spectrum", False):
            try:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines',
                    name=f'Blank Spectrum ({display_timepoint})'
                ))
                fig.update_layout(
        ...
        with col1:
            display_start = st.selectbox(
                "Start timepoint",
                display_options,
                index=0
            )
            blank_start = options_map[display_start]
        with col2:
            # Find index of start point and set end to start+3 by default (if possible)
            start_index = display_options.index(display_start)
            default_end_index = min(start_index + 3, len(display_options) - 1)

            display_end = st.selectbox(
                "End timepoint",
                display_options,
                index=default_end_index
            )
            blank_end = options_map[display_end]
            ...
            try:
                ...
                if cols_between:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        y=avg_blank,
                        mode='lines',
                        name=f'Average Blank ({display_start} - {display_end})'
                    ))
                    fig.update_layout(
...
def calculate_avg_emission_wavelength(df):
    ...
    try:
        ...
        st.error("Error calculating average emission wavelengths")
        return []
...
def calculate_max_emission_wavelength(df):
    ...
    try:
        ...
        st.error("Error calculating max emission wavelengths")
        return []


def augment_dataframe(df, avg_emission_wavelength, integrals, max_emission_wavelength):
    """Combine all calculated metrics into a processed dataframe with consistent time units"""
    try:
        ...
        df_transposed_aew_integral.rename(columns={df_transposed_aew_integral.columns[0]: "Process Time"}, inplace=True)

        # Convert all time values to hours
        try:
            # First try direct conversion
            time_values = pd.to_numeric(df_transposed_aew_integral["Process Time"], errors='coerce')

            # Determine if values are likely in seconds, minutes, or hours
            max_val = time_values.max()

            # Store the original values before conversion
            df_transposed_aew_integral["Original Time"] = df_transposed_aew_integral["Process Time"]

            if pd.notna(max_val):
                if max_val > 1000:  # Likely seconds
                    df_transposed_aew_integral["Process Time [h]"] = time_values / 3600
                    df_transposed_aew_integral["Time Unit"] = "seconds"
                elif max_val > 100:  # Likely minutes
                    df_transposed_aew_integral["Process Time [h]"] = time_values / 60
                    df_transposed_aew_integral["Time Unit"] = "minutes"
                else:  # Already in hours or small values
                    df_transposed_aew_integral["Process Time [h]"] = time_values
                    df_transposed_aew_integral["Time Unit"] = "hours"
            else:
                # If conversion fails, use index as time
                df_transposed_aew_integral["Process Time [h]"] = range(len(df_transposed_aew_integral))
                df_transposed_aew_integral["Time Unit"] = "index"

            # Round to 3 decimal places
            df_transposed_aew_integral["Process Time [h]"] = round(df_transposed_aew_integral["Process Time [h]"], 3)

        except Exception as e:
            # If all else fails, use index values
            df_transposed_aew_integral["Process Time [h]"] = range(len(df_transposed_aew_integral))
            df_transposed_aew_integral["Time Unit"] = "index"
        ...
        return pd.DataFrame(), pd.DataFrame(columns=["Process Time", "Process Time [h]",
...
def closest_times(df, interval):
    ...
    try:
        ...
        st.error("Error finding closest times")
        return df
...
def plot_intensity(df, interval=None, template=template, width=width, height=height, config=config):
    ...
    try:
        ...
        # Remove non-wavelength rows (keep only numerical data)
        non_wavelength_rows = ['Process Time [h]', 'Average emission wavelength [nm]', 'Integral',
                               'Max emission wavelength [nm]', 'Original Time', 'Time Unit']
        rows_to_drop = [col for col in non_wavelength_rows if col in df_plot.index]
        df_plot = df_plot.drop(rows_to_drop)

        fig = go.Figure()

        for i in range(df_plot.shape[1]):
            time_val = df_plot.columns[i]
            fig.add_trace(
                go.Scatter(
                    x=df_plot.index,
                    y=df_plot[time_val],
                    name=f"{time_val}h",
                    customdata=np.tile(time_val, len(df_plot.index)),
                    hovertemplate=
        ...
        st.error("Error creating intensity plot")
        fig = go.Figure()
        fig.update_layout(
            title="Error creating intensity plot",
            xaxis_title="Wavelength [nm]",
            yaxis_title="Intensity"
        )
        return fig
...
def plot_contour(df, ncontours=15, template=template, width=width, height=height, config=config):
    ...
    try:
        ...
        # Convert column names to time in hours if needed
        time_values = []
        for col in numeric_df.columns:
            try:
                val = float(col)
                # Determine time unit and convert to hours
                if val > 1000:  # Likely seconds
                    time_values.append(val / 3600)
                elif val > 100:  # Likely minutes
                    time_values.append(val / 60)
                else:  # Already in hours or small values
                    time_values.append(val)
            except:
                # If conversion fails, use column index
        ...
        Y = np.array(time_values, dtype=float)  # Time points in hours
        ...
        go.Contour(
            ...
            y=Y,  # Y is time in hours
            colorscale='Viridis',
            hovertemplate='Wavelength [nm]: %{x}<br>Time [h]: %{y}<br>Intensity: %{z}<extra></extra>',
            ncontours=ncontours,
        ...
        st.error("Error creating contour plot")
        fig = go.Figure()
        fig.update_layout(
            title="Error creating contour plot",
            xaxis_title='Wavelength [nm]',
            yaxis_title='Time [h]'
        )
        return fig
...
def save_to_excel(header, df, engine='xlsxwriter'):
    ...
    try:
        ...
        st.error("Error saving to Excel")
...
def df_to_txt(df, y_column):
    ...
    try:
        ...
        st.error("Error converting to TXT")
        return ""
